...
def mean_iou_im(outs,target):
    pred = torch.argmax(outs,1).flatten()
    target = target.flatten()
    return mean_iou(pred,target,14)
...
def mean_iou(pred, target, num_classes=None, weights=None, batch=None):
