...
def get_xy(data,gt,mask,image_type,device='cpu'):
    ...
    if image_type == "2d":
        ...
        x,y = gio.sphere2Graph(data,gt=gt,mask=mask,x_only=True,device=device,scale=.75)
    else:
...
class Trainer:
    ...
    def train_one_epoch(self, dataset, graph, mask, image_type, batch_size=1, label="Train"):
        ...
        for x_ref, y_ref in tqdm.tqdm(loader, desc="train", leave=False):
            ...
            #self.check_model()
            #if count % 10 == 1:
            #    self.save_model(os.path.join("segmentation_eval/runs/logs_finetuned", f"checkpoint_{count}.pth"))
            #count+=1
