...
def segment(network,image_type,im,n_classes,mask,device):
    im = im.unsqueeze(0)
        
    graph, metadata = get_graph(im,mask,image_type,device=device)
...
def save_segments(model, image_type, image, gt, mask, n_classes, outname, device):
    og_image = image[:3] * torch.tensor(IMAGENET_STD).view(3, 1, 1) + torch.tensor(IMAGENET_MEAN).view(3, 1, 1)
    og_image = (og_image * 255).byte()
    seg = segment(model, image_type, image, n_classes, mask, device)
...
def main(image_type, device, full_res=True) -> torch.Tensor:
    ...
    if full_res:
        ...
        dataset = Stanford2D3DDataset(config.stanford_dir, datatype="pano", with_depth=True, areas=val_split, transforms=val_transforms)
    ...
    network.load_state_dict(torch.load(config.UNet_finetuned_sphere))
    ...
    #save_segments(network, image_type, best_iou_im, best_iou_gt, mask, dataset.num_classes, config.output_dir+f"{image_type}-best_iou",device)
