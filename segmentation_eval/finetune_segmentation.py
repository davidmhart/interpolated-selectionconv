...
def finetune_segmentation(device, batch_size, max_epochs, fold, trainsplit, logdir, image_type):
    ...
    # Network
    reference_network = UNet(Stanford2D3DDataset.num_classes,in_channels=4)
    reference_network.load_state_dict(torch.load(config.UNet_weights))
    
    network = UNet_graph(Stanford2D3DDataset.num_classes,in_channels=4)
    with torch.no_grad():
    ...
    optimizer = torch.optim.AdamW(network.parameters(),lr=1e-8)
